...
std::uint8_t Memory::getUint8(std::uint16_t index) const {
...
void Memory::setUint8(std::uint16_t index, std::uint8_t value) {
	LOGD << fmt::format("set [{}]: {}", index, value);
	(*_buffer)[index] = value;
}

std::uint16_t Memory::getUint16(std::uint16_t index) const {
...
void Memory::setUint16(std::uint16_t index, std::uint16_t value) {
